...
use safe_nd::{Error, MDataKind, MDataValue};
...
fn create_config_dir(client: &AuthClient, config_dir: &MDataInfo) -> Box<AuthFuture<()>> {
    let config_dir_entries =
        btree_map![KEY_APPS.to_vec() => MDataValue { data: Vec::new(), version: 0 }];
...
fn create_access_container(
    ...
    create_dir(
        ...
        access_container,
        btree_map![
            authenticator_key => MDataValue { version: 0, data: access_cont_value }
        ],
        btree_map![],
    )
    .map_err(From::from)
