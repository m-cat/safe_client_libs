...
//) -> Box<AuthFuture<BTreeMap<Vec<u8>, MDataValue>>> {
//    client
//        .list_seq_mdata_entries(ac_info.name(), ac_info.type_tag())
//        .map_err(From::from)
...
//        .map(|(_, apps)| apps)
//        .and_then(move |apps| {
//            let mut actions = MDataSeqEntryActions::new();
...
//                })?;
//                let mut decoded = access_container::decode_authenticator_entry(&raw.data, &sk)?;
//
...
//                }
//
//                raw.data= access_container::encode_authenticator_entry(&decoded, &sk)?;
//                raw.version += 1;
//                actions = actions.update(auth_key, raw.data.clone(), raw.version);
//            }
//
//            // Update apps' entries
...
//
//                    let mut decoded =
//                        access_container::decode_app_entry(&raw.data, &app.keys.enc_key)?;
//
//                    for name in &container_names {
...
//                    }
//
//                    raw.data = access_container::encode_app_entry(&decoded, &app.keys.enc_key)?;
//                    raw.version += 1;
//                    actions = actions.update(key, raw.data.clone(), raw.version);
//                }
//            }
//
...
//            c3.mutate_seq_mdata_entries(ac_info.name(), ac_info.type_tag(), actions.into())
