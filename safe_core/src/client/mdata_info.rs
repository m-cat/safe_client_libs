...
use safe_nd::{MDataAddress, MDataKind, MDataSeqEntryAction, MDataValue, XorName};
...
pub fn encrypt_entries(
    info: &MDataInfo,
    entries: &BTreeMap<Vec<u8>, MDataValue>,
) -> Result<BTreeMap<Vec<u8>, MDataValue>, CoreError> {
...
pub fn decrypt_values(
    info: &MDataInfo,
    values: &[MDataValue],
) -> Result<Vec<MDataValue>, CoreError> {
...
fn encrypt_value(info: &MDataInfo, value: &MDataValue) -> Result<MDataValue, CoreError> {
    Ok(MDataValue {
...
fn decrypt_value(info: &MDataInfo, value: &MDataValue) -> Result<MDataValue, CoreError> {
    Ok(MDataValue {
