...
use safe_nd::{
    ...
    MDataSeqEntryAction, MDataSeqEntryActions, MDataValue as SndValue, PublicKey, SeqMutableData,
    XorName,
};
            ...
            .or_else(move |error| match error {
                CoreError::NewRoutingClientError(Error::InvalidEntryActions(errors)) => {
                    if attempts < MAX_ATTEMPTS {
                        ...
                        Ok(Loop::Continue((
                            attempts + 1,
                            MDataSeqEntryActions::from(actions),
                        )))
                    } else {
                ...
                CoreError::RequestTimeout => {
                    if attempts < MAX_ATTEMPTS {
                        Ok(Loop::Continue((
                            attempts + 1,
                            MDataSeqEntryActions::from(actions.actions),
                        )))
                    } else {
...
fn update_mdata_entries(
    ...
    name: XorName,
    tag: u64,
    current_entries: &BTreeMap<Vec<u8>, SndValue>,
    desired_entries: BTreeMap<Vec<u8>, SndValue>,
) -> Box<CoreFuture<()>> {
...
fn fix_entry_action(action: MDataSeqEntryAction, error: EntryError) -> Option<MDataSeqEntryAction> {
    match (action, error) {
        ...
        | (MDataSeqEntryAction::Update(value), EntryError::InvalidSuccessor(current_version)) => {
            Some(MDataSeqEntryAction::Update(SndValue {
                data: value.data,
